...
{
	current_node->number = other_node->number;
	current_node->size = other_node->size;
	current_node->sons = new Node* [current_node->size];
	for (int i = 0; i < other_node->number; i++)
	{
		current_node->sons[i] = new Node(other_node->sons[i]->rectangle, current_node);
		BuildTree(current_node->sons[i], other_node->sons[i]);
	}
}

TNaryTree::Node::Node(Rectangle rectangle, Node* parent) : rectangle(rectangle), number(0), size(1), sons(new Node* [size]), parent(parent)
{
	...
	try
	{
		...
		if (tree_path[0] == 'b')
		{
			...
			if (current_number == number - 1)
			{
				...
				current_node->parent->sons[number] = new Node(rectangle, current_node->parent);
		...
		else if (tree_path[0] == 'c')
		{
			if (!current_node->number)
			{
				current_node->sons[0] = new Node(rectangle, current_node);
...
void TNaryTree::Clear(std::string&& tree_path)
{
	try
	{
		if (Empty())
		{
			throw std::invalid_argument("Ther root is empty\n");
			return;
		}
		if (!tree_path.length())
		{
